...
class Form(QtGui.QWidget, Ui_Form):
    def __init__(self, parent=None):
        ...
        # MaxPlus is deprecated
        # TODO remove after testing
        # MaxPlus.NotificationManager.Register(MaxPlus.NotificationCodes.SelectionsetChanged, self.updateSelectionLabel)

        rt.updateSelectionLabel = self.updateSelectionLabel     # Register with MaxScript runtime for use in the callback

        # Add a callback when the selection set changes.
        # The 'selectionSetChanged' event is a known callback event in Max.
        # 'updateSelectionLabel()' is a special way to call a Python function from a callback.
        rt.callbacks.addScript(rt.name('selectionSetChanged'), 'updateSelectionLabel()', id='SelectionChangedCallback')

    def __del__(self):
        # Remove callback when the class is destroyed
        rt.callbacks.removeScripts(id='SelectionChangedCallback')
    ...
    def do_explode(self):
        ...
        if rt.selection.count <= 0:
            ...
            if(self.trimesh_option.isChecked()):
                transformation_set = [node for node in rt.selection.count] # TODO Test this ...
            ...
            rt.redrawViews()
            # close the dialog
            # MaxPlus.NotificationManager.Unregister(MaxPlus.NotificationManager.Handlers[-1]) # TODO
            self.close()
...
def convert_to_triangle_faces(node, addShell, shell_amount, addEditMesh, collapseNode,
    ...
    node = rt.convertToMesh(node)
    # object_state = node.evalWorldState(rt.currentTime)
    # obj_original = object_state.obj
    
    # save node settings to apply on mesh:
    mat = node.transform
    offset_pos = node.objectoffsetpos
    offset_rot = node.objectoffsetrot
    offset_scale = node.objectoffsetscale
    ...
    for face in tri_mesh.Faces:
        ...
        new_face = MaxPlus.Factory.CreateNewTriObject() # TODO
        ...
        for i in range (0,3):
            vert_index = face[i]
            new_obj.verts[i].pos = mesh.verts[vert_index].pos
...
def createPolyFromVertices(vertices, selection):
    ...
    geom = rt.polyMeshObject()
    # geom = MaxPlus.Factory.CreateGeomObject(MaxPlus.ClassIds.PolyMeshObject) # TODO
    ...
    for index, vertex in enumerate(vertex_array):
        mnmesh.V(index).p = rt.point3(vertex.pos.x, vertex.pos.y, vertex.pos.z) # TODO
